...
CREATE TABLE IF NOT EXISTS `marketplace`.`product_image` (
  ...
  `post_date` VARCHAR(45) NOT NULL DEFAULT 'CURRENT_TIMESTAMP',
...
CREATE TABLE IF NOT EXISTS `marketplace`.`comment` (
  ...
  CONSTRAINT `fk_comment_product1`
    ...
    REFERENCES `marketplace`.`product_image` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `marketplace`.`picture`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `marketplace`.`picture` ;
